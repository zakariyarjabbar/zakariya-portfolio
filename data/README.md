...
- Production versioned photos are cacheable for one year. Unversioned public images must revalidate on every request. Next.js handles the existing hashed JavaScript, CSS, fonts, with long-lived caching. Hero and About portraits, plus the social preview, use explicit file-content versions too. Page HTML retains Next.js's normal freshness policy.
